...
public class OperatingSystem {
     ...
     * (u.a. fär Caching etc.), daher -PAGE_SIZE)
     */
    ...
    /**
     * Grääe des RAM in Byte (default: 2^16 Byte = 64 KB RAM)
     */
    private static final int RAM_SIZE = 65536;

    /**
     * Seitengrääe (default: 2^8 Byte = 256 Byte Seitengrääe --> max. 2^8 = 256
    ...
    /**
     * Platteneigenschaften: Plattengrääe = virt. Adressraum reicht hier, weil
     * wir keine weiteren Dateien brauchen
     */
    private static final int DISK_SIZE = VIRT_ADR_SPACE;

    /**
     * Grääe eines Plattenblocks = PAGE_SIZE
     */
     ...
     *
     * @param processSize die Grääe des Prozess-Hauptspeicherbedarfs in Byte
     * @return die neue Prozess-ID oder -1, wenn Erzeugung nicht mäglich
     * (Speichermangel)
     */
    public synchronized int createProcess(int processSize) {
        if (processTable.size() < MAX_NUM_OF_PROCESSES) {
            // RAM-Platz fär neuen Prozess vorhanden
    ...
    public synchronized int write(int pid, int virtAdr, int item) {
        ...
        PageTableEntry pte; // Eintrag fär die zu schreibende Seite

        // äbergebene Adresse präfen
        if ((virtAdr < 0) || (virtAdr > VIRT_ADR_SPACE - WORD_SIZE)) {
            System.err.println("OS: write ERROR " + pid + ": Adresse "
                    + virtAdr
                    + " liegt auäerhalb des virtuellen Adressraums 0 - "
                    + VIRT_ADR_SPACE);
            return -1;
        }
        ...
        if (pte == null) {
            ...
            // Seitenrahmen im RAM fär die neue Seite anfordern und reale
    ...
    /**
     * @param pid
     * @return Prozess-Objekt fär die Prozess-ID
     */
    ...
    /**
     * Behandlung eines Seitenfehlers fär die durch den pte beschriebene Seite
     *
    ...
    private PageTableEntry handlePageFault(PageTableEntry pte, int pid) {
        ...
        testOut("OS: " + pid + " +++ Seitenfehler fär Seite " + pte.virtPageNum);
    ...
    /**
     * Leere RAM-Seite zur Verfägung stellen (ggf. alte Seite auslagern)
     *
     * @param pid Prozess-Id
    ...
    private int getNewRAMPage(PageTableEntry newPte, int pid) {
        // Algorithmus:
        // Anforderung einer neuen RAM-Seite fär die gegebene newPte erfällbar?
        // (< MAX_RAM_PAGES_PER_PROCESS)
        // Ja, Seitenanforderung im RAM ist erfällbar:
        // neue Seite belegen und Adresse zuräckgeben
        // Nein, Seitenanforderung im RAM ist nicht erfällbar:
        // eine alte Seite zur Verdrängung auswählen -->
        // Seitenersetzungs-Algorithmus
        // alte Seite auf Platte auslagern (neuen Diskblock anfordern)
        // im RAM läschen (mit Nullen äberschreiben)
        // Adresse als neue Seite zuräckgeben
        // ----------- Start ----------------
        Process proc; // Aktuelles Prozessobjekt
        ...
        PageTableEntry replacePte; // Eintrag fär eine ggf. zu ersetzende Seite

        proc = getProcess(pid);
        // Anforderung einer neuen RAM-Seite erfällbar?
        if (proc.pageTable.getSize() < MAX_RAM_PAGES_PER_PROCESS) {
            // Ja, Seitenanforderung im RAM ist erfällbar:
            // neue Seite belegen und Adresse zuräckgeben
            newPageFrameAdr = allocateRAMPage();
            // Liste der RAM-Seiten fär den Prozess erweitern
            proc.pageTable.pteRAMlistInsert(newPte);
        } else {
            // Nein, Seitenanforderung im RAM ist nicht erfällbar:
            testOut("OS: getNewRAMPage " + pid + " ++ Seitenfehler fär Seite "
    ...
    /**
     * Liefere eine freie RAM-Seite (Seitenrahmen) und läsche sie aus der
    ...
    /**
     * Läsche eine RAM-Seite und trage sie in die RAM-Freibereichsliste ein.
     * Wird hier nicht verwendet, da ein Prozess keinen Seitenrahmen wieder
     * zuräckgeben muss (keine dynamische Seitenzuteilung).
     *
     * @param ramAdr
     */
    private void freeRAMPage(int ramAdr) {
        // Algorithmus:
        // RAM-Seite mit Nullen äberschreiben (Security!) und neuen
        // Freibereichsblock erzeugen
        // (Eine Zusammenfassung von Freibereichsbläcken (Bereinigen der
        // Fragmentierung) mässte
        // zusätzlich implementiert werden!)
        Integer nullWord; // Null-Speicherwort
        int ri; // aktuelle Speicherwortadresse im RAM
        FreeListBlock ramFB; // neuer FreeListBlock

        // RAM-Seite äberschreiben
    ...
    /**
     * Liefere einen freien Plattenblock und läsche ihn aus der
    ...
    private int allocateDiskBlock() {
        ...
        int freeBlockAdr; // Räckgabeadresse
    ...
    /**
     * Läsche einen Plattenblock und trage ihn in die Platten-Freibereichsliste
     * ein
     *
     * @param diskAdr
     */
    private void freeDiskBlock(int diskAdr) {
        // Algorithmus:
        // Plattenblock mit Nullen äberschreiben (Security!) und neuen
        // Freibereichsblock erzeugen
        // (Eine Zusammenfassung von Freibereichsbläcken (Bereinigen der
        // Fragmentierung) mässte
        // zusätzlich implementiert werden!)
    ...
    // ------------------------- getter-Methoden fär Konstanten
    // -------------------------------
    ...
    /**
     * @return Die Grääe einer Seite (in Byte)
     */
    ...
    /**
     * @return Die Grääe des Hauptspeichers (in Byte)
     */
    ...
    /**
     * @return Die Grääe des virtuellen Adressraums (in Byte)
     */
    ...
    /**
     * @return Die Grääe der Festplatte (in Byte)
     */
    ...
    /**
     * @param testMode - Testausgaben erwänscht?
     */
